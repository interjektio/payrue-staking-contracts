"""Add confirmed type to reward state

Revision ID: ce10bbdbbcee
Revises: 05baa54760ce
Create Date: 2023-12-11 23:02:44.054436
...
revision = "ce10bbdbbcee"
down_revision = "05baa54760ce"
branch_labels = None
depends_on = None


# https://pypi.org/project/alembic-enums/
...
column = Column("reward_distribution", "state", "unsent", "unsent")
enum_migration = EnumMigration(
    op=op,
    enum_name="state",
    old_options=["sent", "sending", "unsent"],
    new_options=["sent", "sending", "unsent", "confirmed"],
    columns=[column],
)


def upgrade():
    enum_migration.upgrade_ctx()


def downgrade():
    enum_migration.downgrade_ctx()
